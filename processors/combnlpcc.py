...
class CombNLPCCProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      lines = self._read_tsv(os.path.join(data_dir, "comb_nlpcc-{}.tsv".format(split)), quotechar='"')

      for (i, line) in enumerate(lines):
        guid = "%s-%s" % (split, i)
        topic = line[1]
        text = line[2]
        context = CombNLPCCProcessor.contexts_dict[topic]
        if split == 'test' and len(line) != 4:
          label = "neutral"
        else:
          label = CombNLPCCProcessor.label_map[str(line[3].strip())]
