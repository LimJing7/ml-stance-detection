...
""" e-fra utils (dataset loading and evaluation) """
...
class EFraProcessor(DataProcessor):
    """Processor for the e-fra dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, f"e-fra-{split}.csv"), quotechar='"', sep=',')

      for (i, line) in enumerate(df.itertuples()):
        guid = "%s-%s" % (split, i)
        topic = line.Target
        text = line.Tweet
        label = EFraProcessor.label_map[line.Stance]
