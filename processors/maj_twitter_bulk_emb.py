...
class MajTwitterBulkEmbProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
        ...
        examples = []
        if self.version == 1:
            df = pd.read_csv(os.path.join(data_dir, f"maj_twitter_bulk_emb-{self.neighbours}-{self.variant}-{split}.csv"), lineterminator='\n')
        elif self.version == 2:
            df = pd.read_csv(os.path.join(data_dir, f"maj_twitter_v2_bulk_emb-{self.neighbours}-{self.variant}-{split}.csv"), lineterminator='\n')

        for (i, line) in enumerate(df.iterrows()):
            guid = "%s-%s" % (split, i)
            topic = line[1]['topic']
            text = line[1]['text']
            context = MajTwitterBulkEmbProcessor.contexts_dict[topic]
...
nlpcc_processors = {
    "stance": MajTwitterBulkEmbProcessor,
}
