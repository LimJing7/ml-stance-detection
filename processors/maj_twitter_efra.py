...
class MajTwitterEFraProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "maj_twitter_efra_en-{}.csv".format(split)), lineterminator='\n')

      for (i, line) in enumerate(df.iterrows()):
        guid = "%s-%s" % (split, i)
        topic = 'macron'
        text = line[1]['text']
        # context = MajTwitterEFraProcessor.contexts_dict[topic]
        label = line[1]['pred']
...
efra_processors = {
    "stance": MajTwitterEFraProcessor,
}
