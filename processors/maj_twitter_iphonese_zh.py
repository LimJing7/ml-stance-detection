...
class MajTwitterIPhoneSEZhProcessor(DataProcessor):
    ...
    label_map = {'AGAINST': 'against',
                 ...
                 'DISCUSS': 'discuss'}
    language = 'zh'
    contexts_dict = {'IphoneSE': 'iPhone SE 是由苹果公司设计销售的智能电话，内部硬件使用iPhone 6s上所使用的苹果A9处理器以及iPhone 6 Plus的收发信号组件、各种感应器等外围组件，外观、屏幕组件则是采用iPhone 5s的，市场定位及定价上属于入门级价格。'}
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "maj_twitter_iphonese_zh-{}.csv".format(split)), lineterminator='\n')

      for (i, line) in enumerate(df.iterrows()):
        guid = "%s-%s" % (split, i)
        topic = 'IphoneSE'
        text = line[1]['text']
        context = MajTwitterIPhoneSEZhProcessor.contexts_dict[topic]
...
nlpcc_processors = {
    "stance": MajTwitterIPhoneSEZhProcessor,
}
