...
class MajTwitterRItaProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "maj_twitter_rita-{}.csv".format(split)), lineterminator='\n')

      for (i, line) in enumerate(df.iterrows()):
        guid = "%s-%s" % (split, i)
        topic = line[1]['topic'][1:-1]  # strip quotation marks
        text = line[1]['text']
...
efra_processors = {
    "stance": MajTwitterRItaProcessor,
}
