...
class MajTwitterWikiContManualProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
        """See base class."""
        examples = []
        df = pd.read_csv(os.path.join(data_dir, f"majority_voted_twitter_wiki_cont_manual-{split}.csv"), lineterminator='\n')
...
maj_wiki_cont_filt_processors = {
    "stance": MajTwitterWikiContManualProcessor,
}
