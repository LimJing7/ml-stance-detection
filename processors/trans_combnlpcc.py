...
class TransCombNLPCCProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      lines = self._read_tsv(os.path.join(data_dir, "trans_comb_nlpcc-{}.tsv".format(split)), quotechar='"')
      ...
      for (i, line) in enumerate(lines):
        ...
        if split == 'test' and len(line) != 4:
          ...
          label = TransCombNLPCCProcessor.label_map[str(line[3].strip())]
...
trans_comb_nlpcc_processors = {
    "stance": TransCombNLPCCProcessor,
}

trans_comb_nlpcc_output_modes = {
    "stance": "classification",
}

trans_comb_nlpcc_tasks_num_labels = {
    "stance": 3,
}
