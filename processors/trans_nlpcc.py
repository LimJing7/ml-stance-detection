...
class TransNLPCCProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      lines = self._read_tsv(os.path.join(data_dir, "trans_nlpcc-{}.tsv".format(split)), quotechar='"')
      ...
      for (i, line) in enumerate(lines):
        ...
        if split == 'test' and len(line) != 4:
          ...
          label = TransNLPCCProcessor.label_map[str(line[3].strip())]
