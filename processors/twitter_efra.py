...
class TwitterEFraProcessor(DataProcessor):
    """Processor for the efra dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "twitter_efra_en-{}.csv".format(split)), lineterminator='\n')
      ...
      for (i, line) in enumerate(df.iterrows()):
        ...
        if split == 'test':
          ...
          label = TwitterEFraProcessor.label_map[str(line[1]['label'].strip())]
