...
class TwitterIPhoneSEProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "twitter_iphonese-{}.csv".format(split)), lineterminator='\n')
      ...
      for (i, line) in enumerate(df.iterrows()):
        ...
        if split == 'test':
          ...
          label = TwitterIPhoneSEProcessor.label_map[str(line[1]['label'].strip())]
        print(f'{topic=}')
        print(f'{text=}')
        print(f'{label=}')
        print(f'{context=}')
...
nlpcc_processors = {
    "stance": TwitterIPhoneSEProcessor,
}
