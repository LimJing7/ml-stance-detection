...
class TwitterIPhoneSEZhProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "twitter_iphonese_zh-{}.csv".format(split)), lineterminator='\n')

      for (i, line) in enumerate(df.iterrows()):
        guid = "%s-%s" % (split, i)
        topic = 'IphoneSE'
        text = line[1]['text']
        context = TwitterIPhoneSEZhProcessor.contexts_dict[topic]
        if split == 'test':
          label = "neutral"
        else:
          label = TwitterIPhoneSEZhProcessor.label_map[str(line[1]['label'].strip())]
...
nlpcc_processors = {
    "stance": TwitterIPhoneSEZhProcessor,
}
