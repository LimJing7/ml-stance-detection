...
class TwitterRItaProcessor(DataProcessor):
    """Processor for the rita dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "twitter_rita-{}.csv".format(split)), lineterminator='\n')
      ...
      for (i, line) in enumerate(df.iterrows()):
        ...
        if split == 'test':
          ...
          label = TwitterRItaProcessor.label_map[str(line[1]['label'].strip())]
