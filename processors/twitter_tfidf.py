...
class TwitterTfIdfProcessor(DataProcessor):
    """Processor for the twitter bulk dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "twitter_tfidf-{}.csv".format(split)), lineterminator='\n')

      for (i, line) in enumerate(df.iterrows()):
        guid = "%s-%s" % (split, i)
        topic = line[1]['search_term']
        text = line[1]['text']
        # context = TwitterBulkProcessor.contexts_dict[topic.lower()]
        if split == 'test':
          label = "neutral"
        else:
          label = TwitterTfIdfProcessor.label_map[str(line[1]['label'].strip())]
...
twttfidf_processors = {
    "stance": TwitterTfIdfProcessor,
}

twttfidf_output_modes = {
    "stance": "classification",
}

twttfidf_tasks_num_labels = {
    "stance": 4,
}
