...
class TwitterWikiCont3PartsProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, f"twitter_wiki_cont_3_part_{self.part}-{split}.csv"), lineterminator='\n', index_col=0)
      ...
      for (i, line) in enumerate(df.iterrows()):
        ...
        if split == 'test':
          ...
          label = TwitterWikiCont3PartsProcessor.label_map[str(line[1]['label'].strip())]
        try:
...
wiki_cont_3_part_processors = {
    "stance": TwitterWikiCont3PartsProcessor,
}

wiki_cont_3_part_output_modes = {
    "stance": "classification",
}

wiki_cont_3_part_tasks_num_labels = {
    "stance": 4,
}
