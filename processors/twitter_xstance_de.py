...
class TwitterXStanceDeProcessor(DataProcessor):
    """Processor for the xstance-de dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      df = pd.read_csv(os.path.join(data_dir, "twitter_xstance_de-{}.csv".format(split)), lineterminator='\n')

      for (i, line) in enumerate(df.iterrows()):
        guid = "%s-%s" % (split, i)
        topic = line[1]['search_term']
        text = line[1]['text']
        # context = TwitterIPhoneSEProcessor.contexts_dict[topic]
        if split == 'test':
          label = "neutral"
        else:
          label = TwitterXStanceDeProcessor.label_map[str(line[1]['label'].strip())]
...
efra_processors = {
    "stance": TwitterXStanceDeProcessor,
}
