...
""" FNC1 utils (dataset loading and evaluation) """
...
class FNC1ZhStanceProcessor(DataProcessor):
    """Processor for the FNC-1 dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      lines = jsonlines.jsonlines.open(os.path.join(data_dir, f"fnc1-{split}.json"))

      for (i, line) in enumerate(lines):
        guid = "%s-%s" % (split, i)
        topic = line['premise']
        text = line['hypothesis']
        label = FNC1ZhStanceProcessor.label_map[line['label']]
    ...
    def get_labels(self):
        """See base class."""
        return ["disagreeing with", "discussing", "agreeing with", "unrelated to"]


fnc1_processors = {
    "stance": FNC1ZhStanceProcessor,
}
