...
""" IBMCS utils (dataset loading and evaluation) """
...
class IBMCSZhStanceProcessor(DataProcessor):
    """Processor for the IBMCS dataset.
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      lines = jsonlines.jsonlines.open(os.path.join(data_dir, f"ibmcs-{split}.json"))

      for (i, line) in enumerate(lines):
        guid = "%s-%s" % (split, i)
        topic = line['premise']
        text = line['hypothesis']
        label = IBMCSZhStanceProcessor.label_map[line['label']]
