...
""" Perspectrum utils (dataset loading and evaluation) """
...
class PerspectrumZhStanceProcessor(DataProcessor):
    ...
    def get_examples(self, data_dir, split='train'):
      """See base class."""
      examples = []
      lines = jsonlines.jsonlines.open(os.path.join(data_dir, f"perspectrum-{split}.json"))

      for (i, line) in enumerate(lines):
        guid = "%s-%s" % (split, i)
        topic = line['premise']
        text = line['hypothesis']
        label = PerspectrumZhStanceProcessor.label_map[line['label']]
...
perspectrum_processors = {
    "stance": PerspectrumZhStanceProcessor,
}

perspectrum_output_modes = {
    "stance": "classification",
}

perspectrum_tasks_num_labels = {
    "stance": 2,
}
