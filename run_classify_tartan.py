...
from tartan_plugin import TartanTrainer
...
def get_meta_grads(args, model, compute_loss, meta_dataloader):
  ## do stuff
  ...
  for idx, i in enumerate(zipped):
    try:
      output_grads.append(torch.mean(torch.stack(i)))
    except TypeError:
...
def train(args, train_datasets, meta_dataset, model, tokenizer, tartan_trainer=None, lang2id=None):
  """Train the model."""
  ...
  train_iterators = [iter(i) for i in train_dataloaders]

  meta_sampler = RandomSampler(meta_dataset) if args.local_rank == -1 else DistributedSampler(meta_dataset)
  meta_dataloader = DataLoader(meta_dataset, sampler=meta_sampler, batch_size=args.train_batch_size)

  len_train_dataloader = sum(len(i) for i in train_dataloaders)
  ...
  n_examples = sum(len(i) for i in train_datasets)
  # Train!
  ...
  train_dl_lens = [len(i) for i in train_dataloaders]
  longest_dl_len = max(train_dl_lens)
  longest_dl_idx = train_dl_lens.index(longest_dl_len)
  ...
  for _ in train_iterator:
    ...
    for longest_only_step, longest_batch in enumerate(epoch_iterator):
      ...
      grads = []
      mlm_grads = []
      for dl_idx, train_iterator in enumerate(train_iterators):
        if dl_idx != longest_dl_idx:
          try:
          ...
          grads.append(grad)
          mlm_grads.append(mlm_grad)
        else:
          loss, mlm_loss, grad, mlm_grad = train_step(args, model, compute_loss, optimizer, longest_batch)
          losses.append(loss)
          mlm_losses.append(mlm_loss)
          grads.append(grad)
          mlm_grads.append(mlm_grad)
      ...
      if args.mlm:
        grads = grads + mlm_grads

      # Fit m-head
      meta_grads = get_meta_grads(args, model, compute_loss, meta_dataloader)

      tartan_trainer.update_weight_gradients(grads, meta_grads)

      # Get the combined gradients
      combo_grads = tartan_trainer.weighted_grads(grads)
      ...
      if (step + 1) % args.gradient_accumulation_steps == 0:
        ...
        tt_weights = tartan_trainer.get_weights()[-1]
        tt_mlm_shift = len(args.train_dataset)
        for i, train_ds in enumerate(args.train_dataset):
          tb_writer.add_scalar(f'tartan_weights/{train_ds}', tt_weights[i], global_step)
          if args.mlm:
            tb_writer.add_scalar(f'tartan_weights/{train_ds}_mlm', tt_weights[i+tt_mlm_shift], global_step)
...
def train_step(args, model, compute_loss, optimizer, batch):
    ...
    if args.mlm:
      mlm_grads = torch.autograd.grad(mlm_loss, model.parameters(), allow_unused=True)
    else:
      mlm_grads = None

    return loss, mlm_loss, grads, mlm_grads
...
def construct_meta_dataset(datasets):
  small_datasets = []
  for dataset in datasets:
    small_datasets.append(dataset[torch.randperm(len(dataset))[:16]])
  combined_dataset = TensorDataset(*map(torch.cat,zip(*small_datasets)))
...
def main():
  ...
  parser.add_argument('--ds_weights', default='equal', choices=['equal', 'inverse_scaled', 'uncorrected_scaled', 'random', 'scaled', 'tartan'], help='how to weight the different datasets')
  ...
  if args.do_train:
    ...
    for lang, train_ds in zip(args.train_language, args.train_dataset):
      ...
      if args.ds_weights == 'tartan':
        dev_dataset = load_and_cache_examples(args, args.task_name, train_ds, tokenizer, split='dev', lang2id=lang2id, evaluate=False, shift=shift)
    ...
    meta_dataset = construct_meta_dataset(dev_datasets)

    if args.ds_weights != 'equal' and args.ds_weights != 'tartan':
    ...
    if args.ds_weights != 'tartan':
      ...
      if args.mlm:
        tartan_trainer = TartanTrainer(args.train_dataset*2, is_meta_tartan=True)
        train_dataset = train_datasets
      else:
        tartan_trainer = TartanTrainer(args.train_dataset, is_meta_tartan=True)
        train_dataset = train_datasets
    global_step, tr_loss, best_score, best_checkpoint = train(args, train_dataset, meta_dataset, model, tokenizer, tartan_trainer=tartan_trainer, lang2id=lang2id)
